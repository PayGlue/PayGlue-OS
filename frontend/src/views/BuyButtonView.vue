...
watch(selectedProductId, (id) => {
  if (useProduct.value && id) formTargetUrl.value = checkoutUrlForProduct(id) ?? ''
})
watch(useProduct, (val) => {
  if (val && selectedProductId.value) formTargetUrl.value = checkoutUrlForProduct(selectedProductId.value) ?? ''
})
...
function startEdit(btn: BuyButtonData) {
  ...
  // Detect if this button is linked to a product (Polar first, then LS, then PayPal)
  ...
  const matchingProduct = matchingPolar ?? matchingLs
  if (matchingProduct) {
    useProduct.value = true
    selectedProvider.value = matchingPolar ? 'polar' : 'lemonsqueezy'
...
onMounted(() => { loadButtons(); loadPolarProducts(); loadLsProducts(); loadPayPalProducts(); loadMappings() })
watch(() => session.activeTenantSlug, () => { loadButtons(); loadPolarProducts(); loadLsProducts(); loadPayPalProducts(); loadMappings() })
</script>
  ...
  <AppShell>
    <div class="space-y-6">
      ...
      <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-semibold uppercase tracking-[0.12em] text-indigo-600">Buy Buttons</p>
        <h1 class="mt-1 text-xl font-semibold text-slate-900">Button creator</h1>
        <p class="mt-1 text-sm text-slate-600">
          One script tag per button. Paste it anywhere — update the label, color, or link here and it changes everywhere instantly, no HTML to touch.
        </p>
      </section>

      <!-- Value prop -->
      <section class="rounded-xl border border-slate-200 bg-slate-50 p-5">
        <p class="mb-1.5 text-sm font-semibold text-slate-700">Why not use the button from Polar, Lemon Squeezy, or PayPal?</p>
        <p class="text-sm leading-relaxed text-slate-500">You can — but each platform gives you a different button with different settings. If you sell through multiple providers, or want to change button text or color, you have to log into each platform separately and update the HTML on every page. PayGlue buttons are provider-agnostic: create them once here, embed a single script tag, and manage everything from one place.</p>
      </section>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <!-- Form -->
        <section
          class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-5"
        ...
        >
          <div class="flex items-center justify-between">
            ...
            <button v-if="isEditing" type="button"
              class="flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-500 hover:border-indigo-300 hover:text-indigo-600"
              @click="resetForm">
          ...
          <!-- Description -->
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">Description <span class="text-slate-400">(optional, shown below button)</span></label>
          ...
          <div class="rounded-lg border p-4 space-y-3 transition-colors"
            :class="useProduct ? 'border-indigo-200 bg-indigo-50/50' : 'border-slate-200'">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium" :class="useProduct ? 'text-indigo-800' : 'text-slate-800'">Link to a product?</p>
                <p class="text-xs mt-0.5" :class="useProduct ? 'text-indigo-500' : 'text-slate-500'">Turn off to use a custom URL (newsletter, event, etc.)</p>
              </div>
              <button type="button"
            ...
            <div v-if="useProduct" class="space-y-2">
              ...
              <p v-if="selectedProvider === 'paypal'" class="text-xs text-slate-400">PayPal subscription plans do not have a direct checkout URL. Enter your PayPal checkout link manually in the URL field below after selecting a plan.</p>
              <p v-else-if="selectedProductId && !checkoutUrlForProduct(selectedProductId)" class="text-xs text-red-600">
          ...
          <div v-if="useProduct && selectedProductId" class="rounded-lg border border-indigo-100 bg-indigo-50/40 p-4 space-y-3">
            <div class="flex items-center justify-between">
            ...
            <p class="text-[10px] text-slate-400">Saved automatically when you save this button.</p>
          </div>
          ...
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">
              Link URL
              <span v-if="useProduct && selectedProductId" class="ml-1 text-indigo-500">(auto-filled from product)</span>
              <span v-else-if="!useProduct" class="ml-1 text-slate-400">(custom link)</span>
            </label>
            <input v-model="formTargetUrl" type="url"
              :placeholder="useProduct ? 'Select a product above' : 'https://example.com/checkout'"
              class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none" />
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="formBgColor" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 p-0.5" />
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="formTextColor" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 p-0.5" />
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <select v-model="formBorderRadius" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:border-indigo-400 focus:outline-none">
          ...
          <div class="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
            <label class="text-xs font-medium text-slate-600">Open link in</label>
            <div class="flex gap-1 rounded-lg border border-slate-200 bg-white p-0.5">
              <button type="button"
                class="rounded px-3 py-1 text-xs font-medium transition-colors"
                :class="formTarget === '_blank' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-800'"
                @click="formTarget = '_blank'">New tab</button>
              <button type="button"
                class="rounded px-3 py-1 text-xs font-medium transition-colors"
                :class="formTarget === '_self' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-800'"
        ...
        <div class="space-y-6">
          ...
          <section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div class="rounded-lg border border-slate-100 bg-slate-50 p-6">
              <div :style="{ display: 'flex', justifyContent: formAlignment === 'center' ? 'center' : formAlignment === 'right' ? 'flex-end' : 'flex-start' }">
                <div>
                  ...
                  <p v-if="formDescription" class="mt-1.5 text-xs text-slate-500" :style="{ textAlign: formAlignment }">{{ formDescription }}</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <section v-if="embedConfig" class="rounded-xl border-2 border-emerald-300 bg-emerald-50 p-5 shadow-sm space-y-4">
            ...
            <p class="text-xs text-emerald-700">Paste this script tag anywhere in your HTML — the button renders exactly where you place it.</p>
            <div>
              <div class="flex items-center justify-between mb-1.5">
                <p class="text-xs font-medium text-emerald-800">Script tag — {{ embedConfig.name }}</p>
                <button type="button"
      ...
      <section class="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div>
            <h2 class="text-sm font-semibold text-slate-900">Saved buttons</h2>
            <p class="text-xs text-slate-500 mt-0.5">Click Edit to get the embed code again or update the button. Changes apply everywhere the button is embedded.</p>
          </div>
          <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600">{{ buttons.length }}</span>
        </div>

        <div v-if="loading" class="px-5 py-8 text-center text-sm text-slate-400">Loading...</div>
        <div v-else-if="buttons.length === 0" class="px-5 py-10 text-center text-sm text-slate-400">
        ...
        <ul v-else class="divide-y divide-slate-100">
          <li v-for="btn in buttons" :key="btn.id"
            ...
            :class="editingId === btn.id ? 'bg-indigo-50' : ''">

            <div class="mt-0.5 h-3 w-3 shrink-0 rounded-full ring-1 ring-slate-200"
              :style="{ backgroundColor: btn.bg_color }" />

            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-slate-900 truncate">{{ btn.name }}</p>
              <p class="text-xs text-slate-500 mt-0.5 truncate">
                {{ btn.label }}
                <span v-if="btn.target_url" class="text-slate-400">&middot; {{ btn.target_url }}</span>
              </p>
            </div>
