...
function startEdit(cfg: PaywallConfigData) {
  ...
  if (cfg.button_url?.startsWith('payglue-table:')) {
    ...
    const mappedProvider = mappings.value.find(m => m.external_product_id === cfg.product_id)?.payment_provider
    if (mappedProvider === 'kofi') selectedProvider.value = 'kofi'
    else if (mappedProvider === 'creem') selectedProvider.value = 'creem'
    else if (mappedProvider === 'patreon') selectedProvider.value = 'patreon'
    else if (lsProducts.value.some(p => p.id === cfg.product_id)) selectedProvider.value = 'lemonsqueezy'
    else if (paypalProducts.value.some(p => p.id === cfg.product_id)) selectedProvider.value = 'paypal'
    else if (gumroadProducts.value.some(p => p.id === cfg.product_id)) selectedProvider.value = 'gumroad'
    else if (paddleProducts.value.some(p => p.id === cfg.product_id)) selectedProvider.value = 'paddle'
...
async function saveConfig() {
  ...
  try {
    ...
    saveErrorPlan.value = isPlanLimitError(err) ? planKeyFromError(err) : null
  } finally {
    saving.value = false
  }
}
...
async function loadGumroadProducts() {
  ...
  try {
    const result = await getGumroadProducts(slug, token)
...
async function loadPaddleProducts() {
  ...
  try {
    const result = await getPaddleProducts(slug, token)
...
async function loadCreemProducts() {
  ...
  try {
    const result = await getCreemProducts(slug, token)
...
async function loadPatreonProducts() {
  ...
  try {
    const result = await getPatreonProducts(slug, token)
...
onMounted(() => { loadConfigs(); loadPolarProducts(); loadLsProducts(); loadPayPalProducts(); loadGumroadProducts(); loadPaddleProducts(); loadCreemProducts(); loadPatreonProducts(); loadPricingTables(); loadMappings() })
watch(() => session.activeTenantSlug, () => { loadConfigs(); loadPolarProducts(); loadLsProducts(); loadPayPalProducts(); loadGumroadProducts(); loadPaddleProducts(); loadCreemProducts(); loadPatreonProducts(); loadPricingTables(); loadMappings() })
...
watch(ctaMode, (val) => {
  if (val === 'product' && selectedProductId.value && !isManualProduct.value) buttonUrl.value = checkoutUrlForProduct(selectedProductId.value)
  else if (val === 'pricing_table' && selectedTableId.value) buttonUrl.value = 'payglue-table:' + selectedTableId.value
  else if (val === 'pricing_table' || (val === 'product' && !isManualProduct.value)) buttonUrl.value = ''
})
...
<template>
  <AppShell>
    <div class="space-y-5">
      ...
      <PageHeader title="Paywall configuration" description="Gate individual Ghost articles behind any payment provider. Ideal if Stripe is not available in your country, or if you want to sell articles as one-time purchases via Polar, Lemon Squeezy, or any other provider of your choice. Configure a paywall per article and paste the snippet into your Ghost HTML card." />

      <div class="grid gap-6 lg:grid-cols-2 [&>*]:min-w-0">

        <!-- Config form -->
        <section class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 space-y-5" :class="isEditing ? 'ring-2 ring-indigo-400' : ''">
          <div class="flex items-center justify-between">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
              {{ isEditing ? 'Edit paywall' : 'New paywall' }}
            </h2>
            <button v-if="isEditing" type="button" class="text-xs text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-200" @click="resetForm">
              Cancel
            </button>
          </div>
          ...
          <div class="rounded-lg border border-slate-200 dark:border-slate-800 p-4 space-y-3">
            <div>
              <p class="text-sm font-medium text-slate-800 dark:text-slate-100 mb-2">Button action</p>
              <div class="flex flex-wrap gap-1.5">
                <button type="button" @click="ctaMode = 'product'"
                  class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                  :class="ctaMode === 'product' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'">
                  Product
                </button>
                <button type="button" @click="ctaMode = 'pricing_table'"
                  class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                  :class="ctaMode === 'pricing_table' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'">
                  Pricing Table
                </button>
                <button type="button" @click="ctaMode = 'custom'"
                  class="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                  :class="ctaMode === 'custom' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200'">
                  Custom URL
                </button>
              </div>
            </div>
            ...
            <div v-if="ctaMode === 'product'" class="space-y-2">
              ...
              <p v-if="selectedProvider === 'paddle'" class="text-xs text-slate-400 dark:text-slate-500">Paddle checkout is driven client-side and has no direct checkout link. Enter your Paddle checkout link manually in the Button URL field below after selecting a product.</p>
            </div>

            <!-- Pricing table picker -->
            <div v-if="ctaMode === 'pricing_table'">
              <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Pricing table</label>
              <p v-if="tablesLoading" class="text-xs text-slate-400 dark:text-slate-500">Loading tables...</p>
              ...
              <p v-if="!tablesLoading && pricingTables.length === 0" class="mt-1 text-xs text-amber-600 dark:text-amber-400">No pricing tables found. Create one first in the Pricing Table section.</p>
              <p class="mt-1.5 text-xs text-slate-400 dark:text-slate-500">The paywall button will open the selected table in an overlay. Visitors can choose their own plan.</p>
            </div>
          </div>

          <!-- Ghost actions mapping (shown when product is linked) -->
          <div v-if="ctaMode === 'product' && selectedProductId" class="rounded-lg border border-indigo-100 dark:border-indigo-500/30 bg-indigo-50/40 dark:bg-indigo-500/10 p-4 space-y-3">
            <div class="flex items-center justify-between">
            ...
            <p class="text-[10px] text-slate-400 dark:text-slate-500">Saved automatically when you save this paywall.</p>
          </div>

          <!-- Headline -->
          <div>
            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Headline</label>
          ...
          <!-- Body -->
          <div>
            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Body text</label>
            <textarea v-model="body" rows="2" placeholder="Purchase access to continue reading."
              class="w-full resize-none rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:border-indigo-400 focus:outline-none" />
          </div>
          ...
          <div v-if="ctaMode !== 'pricing_table'">
            <label class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">
              Button URL
              <span v-if="ctaMode === 'product' && selectedProductId && !isManualProduct" class="ml-1 text-indigo-500">(auto-filled from product)</span>
              <span v-else-if="ctaMode === 'product' && isManualProduct" class="ml-1 text-slate-400 dark:text-slate-500">(paste your Ko-fi page URL)</span>
              <span v-else-if="ctaMode === 'custom'" class="ml-1 text-slate-400 dark:text-slate-500">(custom link)</span>
            </label>
            <input v-model="buttonUrl" type="url" :placeholder="isManualProduct ? 'https://ko-fi.com/yourname' : ctaMode === 'product' ? 'Select a product above' : 'https://example.com/checkout'"
              class="w-full rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:border-indigo-400 focus:outline-none" />
            <p v-if="polarSandbox" class="mt-1.5 text-xs text-amber-600 dark:text-amber-400">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="buttonColor" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 dark:border-slate-800 p-0.5" />
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                <input v-model="textColor" type="color" class="h-9 w-12 cursor-pointer rounded-lg border border-slate-200 dark:border-slate-800 p-0.5" />
          ...
          <div class="grid grid-cols-3 gap-3">
            <div>
              ...
              <select v-model="borderRadius" class="w-full rounded-lg border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:border-indigo-400 focus:outline-none">
        ...
        <div class="space-y-6">
          ...
          <section class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="relative overflow-hidden rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 dark:border-slate-800 dark:bg-slate-800/40">
          ...
          <section class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div class="flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800">
                  <svg class="h-4 w-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" />
                  </svg>
                </div>
                <div>
                  <p class="text-sm font-medium text-slate-800 dark:text-slate-100">Header script</p>
                  <p class="text-xs text-slate-500 dark:text-slate-400">Required once in Ghost Site Header</p>
                </div>
              </div>
              <div class="flex items-center gap-3">
                <span
                  v-if="headerScriptInstalled"
                  class="flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-200 dark:ring-emerald-500/30"
                >
                ...
                <span
                  v-else
                  class="rounded-full bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-700 dark:text-amber-300 ring-1 ring-amber-200 dark:ring-amber-500/30"
                >Not installed</span>
                <RouterLink
                  :to="`/t/${session.activeTenantSlug}/installation`"
                  class="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 hover:underline whitespace-nowrap"
                >
          ...
          <section v-if="step2Config" class="rounded-xl border-2 border-emerald-300 bg-emerald-50 dark:bg-emerald-500/10 p-5 shadow-sm space-y-4">
      ...
      <section class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 px-5 py-4">
          <div>
            <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">Saved paywalls</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Copy the snippet per article and paste into an HTML card in Ghost. Headline, body and button update automatically when you edit, with no need to touch Ghost.</p>
          </div>
          <span class="rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:text-slate-300">{{ savedConfigs.length }}</span>
        </div>

        <div v-if="configsLoading" class="px-5 py-8 text-center text-sm text-slate-400 dark:text-slate-500">Loading...</div>
        <div v-else-if="savedConfigs.length === 0" class="px-5 py-10 text-center text-sm text-slate-400 dark:text-slate-500">
        ...
        <ul v-else class="divide-y divide-slate-100">
          <li v-for="cfg in savedConfigs" :key="cfg.id"
            ...
            :class="editingId === cfg.id ? 'bg-indigo-50 dark:bg-indigo-500/10' : ''">
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-slate-900 dark:text-slate-100 truncate">{{ cfg.name }}</p>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                {{ cfg.product_name || 'Custom URL' }}
                <span v-if="cfg.button_url" class="text-slate-400 dark:text-slate-500">&middot; {{ cfg.button_url }}</span>
              </p>
            </div>
